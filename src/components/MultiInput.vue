  ...
  <div id="main-multi-input">
    ...
    <div class="center">
      <input
        ...
        type="text"
        v-model="inputValue"
        placeholder="Informe o CPF, CNPJ, Raiz CNPJ ou Nome/Razão Social."
        size="80"
        minlength="3"
      ...
      <div v-if="validarRaizCnpj(inputValue)">
        <input
          type="checkbox"
          v-model="pesquisarPorRaizCnpj"
          @change="validateInput"
        > Deseja pesquisar pela raiz do CNPJ.<br>
      </div>
      <div v-if="inputValue.length >= 3 && inputValue.match(/[A-Zi]/i) !== null">
        <span
          v-for="tipo in tipos"
          :key="tipo.idTipoPessoa"
        >
          <input
            type="radio"
            name="tipoPessoa"
            :id="tipo.idTipoPessoa"
            v-model="tipoPessoa"
            :value="tipo.idTipoPessoa"
          >
	...
	methods: {
		...
		validateInput () {
			...
			if (this.inputValue.match(regexRegra1) !== null) {
				...
				this.cpf = "";
				this.cnpj = "";
				this.cnpjRaiz = Vue.prototype.$validations;
			...
			return Vue.prototype.$validations.validateInput();
		},
		validarCPF (pCpf) {
